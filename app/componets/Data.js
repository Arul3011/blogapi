 ...
 const handeldelet = async(id)=>{
  ...
  return (
    <div className='maindiv'>
       <div className="top">
       <input type="text" onChange={(e)=>setTask(e.target.value)} value={task}/> <button onClick={handeladdtask}>add</button>
       </div>
      <ul>
        {
             fetdata.length===0 ? <p className='pp'>{load ? "Loading..." : "ADD YOUR TODO"}</p> : fetdata.map((val) => {
                return(
                    <li key={val._id} className='col'>
                  <input type="checkbox" onChange={()=> handelcheck(!val.checked,val._id)} checked={val.checked}  />
                <p style={{  textDecoration:val.checked? "line-through" : "none"}}>{val.task}</p>
                <button onClick={()=> handeldelet(val._id)}>del</button>
              </li>)
    })
        }
      </ul>
